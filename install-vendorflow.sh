#!/bin/bash
# VendorFlow Complete Installer
# This script creates all view components and deployment files

set -e
echo "Installing VendorFlow Frontend Components..."

# Create directories
mkdir -p views components

# Create LogViewer
cat > components/LogViewer.tsx << 'LOGVIEWER_EOF'
...
export const LogViewer: React.FC<LogViewerProps> = ({ logs }) => {
  ...
  const getLogIcon = (type: LogEntry['type']) => {
    switch (type) {
      case 'success': return <CheckCircle className="w-3 h-3 text-green-400 flex-shrink-0" />;
      ...
      default: return <Activity className="w-3 h-3 text-blue-400 flex-shrink-0" />;
    }
  };
...
echo "✓ LogViewer created"

# I'll create a compressed version with all views in one command...
